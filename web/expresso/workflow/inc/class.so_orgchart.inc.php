...
class so_orgchart
{
	...
	private function _checkAccess($organizationID = null, $checkType = false, $safeMethod = false)
	{
		...
		$authorized = false;
		if ($checkType)
			$authorized = $this->acl->checkUserAccessToType('ORG', $this->userID);
		else
		{
			if (!is_numeric($organizationID))
				$authorized = false;
			else
				$authorized = $this->acl->checkUserAccessToResource('ORG', $this->userID, (int) $organizationID);
		}
	...
	function getLocal($organizationID)
	{
		...
		$query_fields = 'organizacao_id, localidade_id, centro_custo_id, descricao, empresa, endereco, complemento, cep, bairro, cidade, uf';
	 ...
	 * @access public
	 */
	function addLocal($organizationID, $costCenter, $description, $company, $address, $complement, $zipCode, $neighborhood, $city, $state )
	{
		$this->_checkAccess($organizationID);

		$query = "INSERT INTO localidade(organizacao_id, centro_custo_id, descricao, empresa, endereco, complemento, cep, bairro, cidade, uf) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$result = $this->db->query($query, array($organizationID, $costCenter, $description, $company, $address, $complement, $zipCode, $neighborhood, $city, $state ));
		$this->_checkError($result);
	 ...
	 * @access public
	 */
	function updateLocal($organizationID, $costCenter, $description, $localID, $company, $address, $complement, $zipCode, $neighborhood, $city, $state )
	{
		$this->_checkAccess($organizationID);

		$query = "UPDATE localidade SET organizacao_id = ?, centro_custo_id = ?, descricao = ?, empresa = ?, endereco = ?, complemento = ?, cep = ?, bairro = ?, cidade = ?, uf = ? WHERE (localidade_id = ?)";
		$result = $this->db->query($query, array($organizationID, $costCenter, $description, $company, $address, $complement, $zipCode, $neighborhood, $city, $state, $localID) );
		$this->_checkError($result);
