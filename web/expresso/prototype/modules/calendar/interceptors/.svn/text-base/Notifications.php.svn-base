...
class Notifications extends Helpers {
	...
	self::sendMail($data, $ical, $part['user']['mail'], $subject, 'notify_response_delegated_status_body');
    }
	...
	if ((Config::regGet('ical://' . $schedulable['id'] . '/' . $method) === false) || ($method == 'CANCEL')) { //Verifica se o ical ja não esta no reg
	    ...
	    $ical['source'] = Controller::format(array('service' => 'iCal'), array($schedulable), array('method' => $method));
	    $ical['type'] = 'text/plain';
	...
	switch ($temp) {
		...
		foreach ($schedulable['participants'] as $ii => $participant) {
		    if (Config::me('mail') !== $participant['user']['mail'])
	...
	self::sendMail($data, $ical, $part['user']['mail'], $subject, !$delegatedParams ? 'notify_create_body' : 'notify_create_delegated_body', $organizer);
    }
	...
	if ($delegatedParams) {
	    $data = array_merge($data, $delegatedParams);
	    $tplDelegated = 'notify_delegated_status_body';
	}

	switch ($type) {
	    case STATUS_ACCEPTED:
		$tpl = !$delegatedParams ? 'notify_accept_body' : $tplDelegated;
		$subject['notificationType'] = 'Convite Aceito';
		break;
	    case STATUS_TENTATIVE:
		$tpl = !$delegatedParams ? 'notify_attempt_body' : $tplDelegated;
		$subject['notificationType'] = 'Convite  aceito provisoriamente';
		break;
	    case STATUS_CANCELLED:
		$tpl = !$delegatedParams ? 'notify_reject_body' : $tplDelegated;
		$subject['notificationType'] = 'Convite rejeitado';
		break;
	    case STATUS_DELEGATED:
		if ($delegatedParams)
		    $data = array_merge($data, $delegatedParams);
		$tpl = 'notify_delegated_body';
	...
	if ((Config::regGet('noAlarm') === false) && (self::futureEvent($schedulable['startTime'], $schedulable['rangeEnd'], $schedulable['id']))) {
	    ...
	    if (count($part) > 0)
		self::sendMail($data, $ical, implode(',', $part), $subject, 'notify_cancel_body');
	}
    }
	...
	if ((Config::regGet('noAlarm') === false) && (self::futureEvent($schedulable['startTime'], $schedulable['rangeEnd'], $schedulable['id']))) {
	    ...
	    self::sendMail($data, $ical, $participant['user']['mail'], $subject, 'notify_cancel_body');
	}
    }
	...
	if (($alt === true) && (Config::regGet('noAlarm') === false) && (self::futureEvent($schedulable['startTime'], $schedulable['rangeEnd'], $schedulable['id']))) {
		...
		self::sendMail($data, $ical, implode(',', $part), $subject, 'notify_modify_body');
	}
    }
