function contentMenu(){
	...
	$('.fullcalendar-context-menu').bind('contextmenu', function(event){
		...
		var idEvent = false, idRecurrence = false, idCalendar = false;
		...
		var template = DataLayer.render( 'templates/menu_context_event.ejs', {event: schedulable.id ,
                    top: (event.clientY - 135 + top), left: (event.clientX - 445), 
		    signature: Calendar.signatureOf[idCalendar],
                    calendars: Calendar.calendars, isRecurrence: (!isRecurrence || isRecurrence == 'none') ? false : true,
