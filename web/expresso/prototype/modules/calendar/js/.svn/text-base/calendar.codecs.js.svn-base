...
DataLayer.codec( "calendarSignature", "calendar", {
	...
	return $.map( signatures, function( signature ){
		...
		events: function( start, end, callback ){
			...
			Calendar.currentView = DataLayer.get( 'schedulable:calendar', {
			    start: start, 
			    end: end
			} );
		    }
...
/*Todo Melhorias serão feitas na API*/
...
DataLayer.codec( "calendarSignature", "configure", {
	...
	var res = $.map(evtObj, function( form ){
		...
		user: User.me.id,	
		calendar: {
		    id: Calendar.calendarOf[form.signature].id,
...
function decodeRepeat ( form ) {
	...
	switch(form.frequency) {
		...
		break;
	    case 'yearly':
		patati['byyearday'] = (1 + date.getDayOfYear());
		break;
	    default :
		return patati;
	}
...
DataLayer.codec( "schedulable", "detail", {
	...
	var res =  DataLayer.encode('schedulable:preview', evtObj);
...
DataLayer.codec( "schedulable", "calendar", {
	...
	$.map(filtered, function( evt ){
		...
		if(Calendar.signatureOf[calendar.id].isOwner == "1"){
			...
			attendee = DataLayer.get('participant', evt.participants[i]);
			if(attendee.user == User.me.id)
			    return (attendee.acl.indexOf('w') >= 0 || attendee.acl.indexOf('o') >= 0 ) ? {
				selectable: true, 
				className: 'fullcalendar-context-menu  event-id-'+idEvent+' calendar-id-'+calendar.id+ (isRecurrence ? ' isRecurrence Recurrence-id-'+Recurrence : '')
				} : {
				editable: false, 
				selectable: true ,
		    ...
		    return {
			editable: false, 
			className: 'blocked-event-permision  fullcalendar-not-context-menu event-id-'+idEvent+' calendar-id-'+calendar.id,  
			selectable: true, 
			disableDragging: isRecurrence
		    };
		}
		else{
		    ...
		    return DataLayer.merge({
			editable: aclSignature.acl['write'] && !isRecurrence,
	    ...
	    var duration = parseInt( evt.endTime ) - parseInt( evt.startTime ), isRepeat = false;
		...
		occurrences[ occurrences.length ] = evt.startTime;
		...
		grouped[ evt.calendar ].push( DataLayer.merge({
		    ...
		    start: Timezone.getDateCalendar(new Date( parseInt( occurrences[i] ) ), calendar.timezone,  !!evt.DayLigth ? evt.DayLigth.calendar.startTime : Timezone.daylightSaving),
		    end:   Timezone.getDateCalendar(new Date( parseInt( occurrences[i] ) + duration - (!!parseInt(evt.allDay) ? 86400000 : 0)), calendar.timezone, !!evt.DayLigth ? evt.DayLigth.calendar.startTime : Timezone.daylightSaving),
		    allDay: parseInt( evt.allDay ),
		    isRepeat: isRepeat,
		    occurrence: i,
		    calendar: evt.calendar
		    }, eventEditable(evt.id, isRepeat, i ) ) );
	});
	...
	return {
	    filter: ['AND', 
			['OR', 
			    ['AND', 
				['>=', 'rangeEnd', filter.start.getTime()], 
				['<=', 'rangeEnd', filter.end.getTime()] , 
			    ],
			    ['AND', 
				['>=', 'rangeStart', filter.start.getTime()], 
				['<=', 'rangeStart', filter.end.getTime()] , 
			    ],
			    ['AND', 
				['<=', 'rangeStart', filter.start.getTime()], 
				['>=', 'rangeEnd', filter.end.getTime()] , 
			...
			['IN', 'calendar',  Calendar.calendarIds]
		], 
	    criteria: {
		deepness: 2, 
		timezones: timezone
