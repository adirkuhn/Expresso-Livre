...
function updateMap(data){
	...
	var event = $("#new_event_map").fullCalendar('clientEvents', 'map')[0];
		...
		if(!!event){	
			...
			$("#new_event_map").fullCalendar( 'updateEvent', event )
		}
...
function disponibily(objEvent, path, attendees){
	var formatString = User.preferences.dateFormat + " " + User.preferences.hourFormat;
	...
	$("#calendar_addevent_details7").html(mapHtml);
	$("#new_event_map").fullCalendar(DataLayer.merge({
		height : 300,
		...
		events: [
			{
				...
				editable : objEvent.acl.write || objEvent.acl.organization
			}
		],
	...
	var updateMapView = function(){
		updateMap();
		var view = $("#new_event_map").fullCalendar('getView');
		var map = {};
		$("#new_event_map").fullCalendar( 'removeEventSource', eventSource );
		...
		if(eventSource.length)
			$("#new_event_map").fullCalendar( 'addEventSource', eventSource );
	}; 
	$('[href="#calendar_addevent_details7"]').click(function(eventData, eventObject){
		$("#new_event_map").fullCalendar( 'gotoDate', Date.parseExact($('[name="startDate"]').val() + " " + $.trim($('[name="startHour"]').val()), formatString) , formatString);
		//if(eventObject == undefined)
			updateMapView();
	});	
	$("#new_event_map").find(".fc-button-prev, .fc-button-next, .fc-button-today").click(function(){
		updateMapView();
	});
}
