	...
	*/
	public function formatSieveScript( $rules )
    {
		...
		{	
			...
			($i >0) ? $script_match = 'els' : $script_match = '';
			...
			{					
				...
				switch($criteria[$j]['field']) {
					...
					case 'subject':
						$criteria[$j]['field'] = "\"" . $criteria[$j]['field'] . "\"";
						$criteria[$j]['value'] = "" . imap_8bit($criteria[$j]['value']) . "*\", \"" . base64_encode($criteria[$j]['value']) . "";
						$criteria[$j]['operator'] = "^";  //temporary
				...
				switch ($criteria[$j]['operator']) {
					...
					/* 
					   TO-DO:
					   Inconsistência na regra do "não contém".
					*/
					case '!*':
						$criteria[$j]['operator'] = "contains";
						$criteria[$j]['value'] = "[\"" . $criteria[$j]['value'] . "\"]";
			...
			//$script_action = " {\r\n ";
			/* Somente para controlar que o addFlag virá antes de qualquer ação. Modificar isso depois */
			$action_addFlag = '';
			...
			foreach ($action as $k => $value)
			{
				switch ($action[$k]['type']) {
					...
					case 'vacation':
						...
						$action[$k]['parameter'] = "\"" . $action[$k]['parameter'] . "\"";
						$vacation_action = ' :subject "Fora do Escrit&oacuterio" ' . $action[$k]['parameter'] . ";";
						$vacation = true;
						continue;
					case 'discard':
						break;
