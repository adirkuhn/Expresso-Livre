...
class ContactsResource extends Resource {
	...
	$h = new Hypermedia();
	$c = new Collection($request->resources, 'ContactsResource');

	try {
	    $this->secured();

            $contacts = Controller::find( array( 'concept' => 'contact' ), false, array( 'filter' => array( '=', 'user',  Config::me("uidNumber") ) ) );
		...
		$d = new Data();
                $i = new Item($request->resources, 'ContactsResource', $value['id']);
		$d->setName('id');
                $d->setValue($value['id']);
