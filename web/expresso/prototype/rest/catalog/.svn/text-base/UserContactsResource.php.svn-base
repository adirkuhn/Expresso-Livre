...
//Recupera os contatos pessoais do usuario
		...
		foreach ($contacts as $value) {
			...
			$d = new Data();
			$i = new Item($request->resources, (($value['user'] == Config::me("uidNumber")) ? 'ContactsResource' : 'SharedContactResource'), $value['id']);
			$d->setName('id');
			$d->setValue($value['id']);
    ...
    private function isEnabledDynamicContacts($user){
        ...
        $sql = 'SELECT preference_value '.
                ...
                'WHERE preference_app = \'expressoMail\' AND '.
                        'preference_owner = ' . $user;

        $preference = Controller::service('PostgreSQL')->execResultSql($sql, true);

        if(count($preference) > 0 && isset($preference['preference_value'])){
                $preference = unserialize( $preference['preference_value'] );

                if(isset($preference['use_dynamic_contacts'])){
                        $preference = $preference['use_dynamic_contacts'];
                        return ($preference == '1');
                }
        }
        return false;
