...
function edit_shared_account(uid)
{
	...
	if (data.status == 'true')
	{
		...
		if(calendarPermission){
			...
			if(!current[calendarPermission[i].user.uid ]){
...
cShareMailbox.prototype.getaclfromuser = function(user)
{
    ...
    {
	if ( this.ownersAcl[user].indexOf('lrs',0) >= 0)
	{
	...
	if ( this.ownersAcl[user].indexOf('d',0) >= 0)
	{
	    Element('em_input_deleteAcl').checked = true;
	}
	if ( this.ownersAcl[user].indexOf('wi',0) >= 0)
	{
	    Element('em_input_writeAcl').checked = true;
	}		
	if (this.ownersAcl[user].indexOf('a',0) >= 0)
	{
	    Element('em_input_sendAcl').disabled = false;
	...
	if ( this.ownersAcl[user].indexOf('c',0) >= 0)
	{
	    Element('em_input_folderAcl').checked = true;
	}
	if( Element('em_input_writeAcl').checked && Element('em_input_sendAcl').checked ){
	    Element('em_input_saveAcl').disabled = false;
