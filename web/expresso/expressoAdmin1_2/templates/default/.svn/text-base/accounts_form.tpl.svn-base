...
<div id="tabcontent1" class="inactivetab">
	<table width="90%" border="0" cellspacing="4">
		<tr bgcolor={row_on}>
			...
			<td><input type="text" id="organization_search" {disabled} autocomplete="off" size=20 onKeyUp="javascript:search_organization(this.value, 'ea_combo_org_info');" onBlur="javascript:sinc_combos_org(context.value); get_available_sambadomains(context.value, '{type}')"></td>
		</tr>

		<tr bgcolor={row_off}>
			<td>{lang_organizations}:</td>
			<td><select {disabled} id="ea_combo_org_info" name="context" onchange="javascript:sinc_combos_org(this.value); get_available_groups(this.value); get_available_maillists(this.value); get_available_sambadomains(this.value, '{type}')">{sectors}</select></td>
		</tr>
