...
//		var $debug = True;
		...
		function manager()
		{
			...
			if ($_POST['add'])
			{
				...
				if ($alarm_time <= time())
				{
					echo	'<center>'.lang('Alarm is older than now!!!').'</center><center>'.
							'<form method="POST" action="'.$GLOBALS['phpgw']->link('/index.php',Array('menuaction'=>'calendar.uialarm.manager')).'">'.
							'<input type="hidden" value="'.$this->bo->cal_id.'" name="cal_id">'.
							'<input type="hidden" value="'.$_POST['return_to'].'" name="return_to">'.
							'<center><input type="submit" value="'.lang("Back").'"/></center></form></center>' ;
					$GLOBALS['phpgw']->common->phpgw_exit(True);
				}
				...
				foreach ( $this->bo->bo->so->cal->event['alarm'] as $object ){
					...
					{
						echo '<center>'.lang("Alarm already added!!!").'</center><center>'.
							 '<form method="POST" action="'.$GLOBALS['phpgw']->link('/index.php',Array('menuaction'=>'calendar.uialarm.manager')).'">'.
							 '<input type="hidden" value="'.$this->bo->cal_id.'" name="cal_id">'.
							 '<input type="hidden" value="'.$_POST['return_to'].'" name="return_to">'.
							 '<center><input type="submit" value="'.lang("Back").'"/></center></form></center>' ;
				...
				if ($time == 0)
				{
					echo '<center>'.lang("Invalid Alarm!!!").'</center><center>'.
							 '<form method="POST" action="'.$GLOBALS['phpgw']->link('/index.php',Array('menuaction'=>'calendar.uialarm.manager')).'">'.
							 '<input type="hidden" value="'.$this->bo->cal_id.'" name="cal_id">'.
							 '<input type="hidden" value="'.$_POST['return_to'].'" name="return_to">'.
							 '<center><input type="submit" value="'.lang("Back").'"/></center></form></center>' ;
