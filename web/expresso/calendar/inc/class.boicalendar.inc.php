...
class boicalendar {
	...
	function read_line_unfold($ical_text) {
		if ($this->line < count($ical_text)) {
			...
			// Corrige a codificacao dos caracteres, caso haja suporte a mesma.
			$str = trim($str);
			if (mb_detect_encoding($str, 'auto') == 'UTF-8')
				if ($conv = preg_replace("/([\xC2\xC3])([\x80-\xBF])/e", "chr(ord('\\1')<<6&0xC0|ord('\\2')&0x3F)", $str))
					$str = $conv;
	...
	function is_owner($part_record) {
		if (($part_record['user'] . '@' . $part_record['host'] == $GLOBALS['phpgw_info']['user']['preferences']['email']['address']) || ($part_record['cn'] == $GLOBALS['phpgw_info']['user']['account_lid']) ||
			($part_record['mailto']['user'] == $GLOBALS['phpgw_info']['user']['account_lid'])) {
				return True;
		} else {
				return False;
			}
	...
	function import_file() {
		...
		$newfilename = md5($_FILES['uploadedfile']['tmp_name'] . ", " . $uploadedfile_name . ", " . time() . getenv("REMOTE_ADDR") . $random_number);
	...
	function safeBitCheck($number, $comparison) {
		...
		if (($str <= strlen($binComparison)) && ($binComparison {
			$str -1 }
		=== "1"))
		return '1';
else
...
function import($mime_msg = '', $from_ajax = false, $importAccount = false) {
	/*
	 * Caso seja uma importação para conta compartilhada carrega as acls do usuario
	 */
	if ($importAccount != false) {
		...
		$where = array (
			...
			'acl_account' => $importAccount['uidnumber'],
			'acl_location' => $GLOBALS['phpgw_info']['user']['account_id']
		);
		$db = $GLOBALS['phpgw']->db;
		$db->select('phpgw_acl', $colunas, $where, null, null);
		$row = $db->row(true);

		if (!$this->safeBitCheck(2, $row['acl_rights'])) //Se o usuario não tem permisão de adicionar eventos
			return 'You do not have permission to add events';
	}
	 ...
	 * Caso seja um Reply.
	 */
	if (strtoupper($ical['method']['value']) == 'REPLY') {
		//atualiza evento caso seja evento externo
		if (isset ($ical['event']['0']['x_type']))
			foreach ($ical['event']['0']['x_type'] as $property)
				if ($property['name'] === 'MICROSOFT_CDO_IMPORTANCE')
					return $this->updateHour($ical);

		//Ignora respostas externas
		return 'The import was executed successfully.';
				}

	/*-------------------------------------------------------------*/

	/*
	 * Ativa a flag de cancelamento caso o ical for do method CANCEL
	 */
	$cancelEvent = false;
	if (strtoupper($ical['method']['value']) == 'CANCEL')
		$cancelEvent = true;
	/*-------------------------------------------------------------*/
	...
	for ($i = 0; $i < $c_events; $i++) {
		$ex_participants = new exParticipants();

		if ($ical['event'][$i]['uid']['value'])
			$uid_exists = $so_event->find_cal_id($ical['event'][$i]['uid']['value']);
				else
					$uid_exists = False;

		if ($cancelEvent) // If cancelamento
				{
			if ($importAccount != false && !$this->safeBitCheck(8, $row['acl_rights'])) // se for em uma conta compartilhada e se tem permisão de deletar
				return 'You do not have permission to delete events';

			if ($uid_exists) // If o evento existe na agenda
				$so_event->delete_entry($uid_exists); // Adicona o id a lista de eventos a deletar

			continue; // Continua o for nenhum processamento mais é nescessario
				}
		...
		if ($ical['event'][$i]['ex_participants']['value']) {
			$ex_participants->setParticipantsByString($ical['event'][$i]['ex_participants']['value']);
					}
		...
		if (isset ($ical['event'][$i]['attendee'])) {
			...
			for ($a = 0; $a < $c_attendees; $a++) {
				...
				if ($email) {
								// Remove os espaÃ§os, tabs e quebra-de-linha no dominio
			...
			$ex_participants->setParticipantsByString($all_attendees);
		...
		if (isset ($ical['event'][$i]['rrule']) OR isset ($ical['event'][$i]['duration'])) {
			...
			elseif (isset ($ical['event'][$i]['duration'])) {
				...
				$ptimer = mktime($ical['event'][$i]['dtstart']['hour'], $ical['event'][$i]['dtstart']['min'], $ical['event'][$i]['dtstart']['sec'], $ical['event'][$i]['dtstart']['month'], $ical['event'][$i]['dtstart']['mday'], $ical['event'][$i]['dtstart']['year']);
						...
						* (this define in "start" tag) and next day  :(
							...
							*/
							...
							// Weeks::Day--
				if ($ical['event'][$i]['duration']['weeks'] AND ($ical['event'][$i]['duration']['hours'] == 0) AND ($ical['event'][$i]['duration']['minutes'] == 0)) {
					$ical['event'][$i]['duration']['days'] = ($ical['event'][$i]['duration']['days'] + ($ical['event'][$i]['duration']['weeks'] * 7)) - 1;
					unset ($ical['event'][$i]['duration']['weeks']);
							...
							// Days::Day--
				if ($ical['event'][$i]['duration']['days'] AND ($ical['event'][$i]['duration']['hours'] == 0) AND ($ical['event'][$i]['duration']['minutes'] == 0)) {
				...
				list ($recur_enddate['year'], $recur_enddate['month'], $recur_enddate['mday'], $recur_enddate['hour'], $recur_enddate['min'], $recur_enddate['sec']) = preg_split('/:/', date("Y:m:d:H:i:s", $enddate));
							...
							// Set End of event
				$so_event->set_end($recur_enddate['year'], $recur_enddate['month'], $recur_enddate['mday'], $recur_enddate['hour'], $recur_enddate['min'], $recur_enddate['sec']);
		...
		// Owner
		$organizer = $ical['event'][$i]['organizer'];

		if (isset ($organizer)) {
			if ($this->is_owner($organizer) || $organizer == -2) {
				$so_event->add_attribute('owner', $GLOBALS['phpgw_info']['user']['account_id']);
			} else {
				$so_event->add_attribute('owner', -2);
				$aux = explode(':', $organizer['cn']);
				$so_event->add_attribute('organizer', '' . $aux[0] . ' <' . $organizer['mailto']['user'] . '@'. $organizer['mailto']['host'].'>');
					}
					}

		// se for importação para conta compartilhada
		$user = (int) ($importAccount ? $importAccount['uidnumber'] : $GLOBALS['phpgw_info']['user']['account_id']);
		$userMail = $importAccount['mail'] ? $importAccount['mail'] : $GLOBALS['phpgw_info']['user']['email'];

		$so_event->add_attribute('participants', 'A', $user);
					$event = $so_event->get_cached_event();

		/*
		 * Remove Email do participantes externos
		 */
		$usersToRemove = array ();
		$usersToRemove[] = $user;
		foreach ($event['participants'] as $auxi=> $v)
			$usersToRemove[] = $auxi;

		$ldapService = ServiceLocator :: getService('ldap');
		foreach ($usersToRemove as $v) {
			$ex_participants->removeParticipant($ldapService->getMailByUidNumber($v));
			$alternates = $ldapService->getMailAlternateByUidNumber($v);
			foreach ($alternates as $ii => $vv)
				$ex_participants->removeParticipant($vv);
		}
		$ex_participants->removeParticipant($event['organizer']);
		/// -------------------------------------------------------- ///

		$event['ex_participants'] = $ex_participants->getParticipantsSerializable();

		if (!isset ($this->bo)) {
			require_once (dirname(__FILE__) . '/class.bocalendar.inc.php');
			$this->bo = new bocalendar();
		}

		if ($c_events < 2) //Ignora verificação de conflito quando o ical é de varios eventos
		{
			if ($overlapping_events = $this->bo->event_overlap($event, (!$from_ajax), null, $user)) {
				if ($_GET['calendarImport'] == 1) //caso venha do modulo calendar
					{
					$url = 'expressoMail1_2/controller.php?action=' . implode("&", array (
						'calendar.uicalendar.overlap',
						'o_events=' . implode(';', $overlapping_events),
						'this_event=' . $event['id'],
						'this_account=' . $user
					));
		...
		if ($importAccount)
			$so_event->add_entry($event, true, $importAccount);
		else
	...
	/*
	 * Exclue os eventos que estão na lista para serem deletados
	 */
	if ($cancelEvent)
		$so_event->expunge();
	/*----------------------------------------------------------*/
	...
	} else // checando se o icalendar veio no formato esperado, ou seja, foi parseado corretamente
		if (!empty ($ical))
				return true;
		else
			return 'Cannot import target event. Please check if the format is right.';

}

function updateHour($ical) {
	...
	$cal_id = $so_event->find_cal_id($ical['event'][0]['uid']['value']);

	//Agendamento nao ecnontrado
	if (!$cal_id)
		return 'The event was not found in your calendar';

	$event = $so_event->read_entry($cal_id);
	...
	@ reset($datetime_vars);
	while (list ($e_datevar, $i_datevar) = each($datetime_vars)) {
		if (isset ($ical['event'][0][$i_datevar])) {
			$temp_time = $so_event->maketime($ical['event'][0][$i_datevar]);
			@ reset($date_array);
			while (list ($key, $var) = each($date_array))
				$event[$e_datevar][$var] = (int) (date($key, $temp_time));
	...
	$so_event->add_entry($event, false);
	$bo_event = createobject('calendar.bocalendar');
	$bo_event->send_update(MSG_MODIFIED, $event['participants'], $event, $event);
...
function updateExParticipantState($cal_uid, $mail, $situation, $cn = false) {
	$db = $GLOBALS['phpgw']->db;
	$query = 'SELECT ex_participants FROM phpgw_cal WHERE uid = \'' . trim($cal_uid) . '\'';
	if (!$db->query($query))
		return false;
	$row = $db->row(true);
	...
	$exParticipants->setParticipantsBySerializable($row['ex_participants']);
	$exParticipants->updateParticipant($mail, $situation, $cn);
	$query = 'UPDATE phpgw_cal SET ex_participants = \'' . $exParticipants->getParticipantsSerializable() . '\' WHERE uid = \'' . trim($cal_uid) . '\'';
	if (!$db->query($query))
		return false;
