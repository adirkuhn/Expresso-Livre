...
if(isset($_SESSION['isCrun']))
	$_SESSION['isCrun'] = false;
...
de repeticao escolhido pelo usuario (diario, semanal, mensal, anual) e insere alarmes nas respectivas repeticoes do evento. */
		...
		*/
		...
		function fetch_event($event_id,$options='')
		{
			...
			if($this->stream->num_rows() > 0)
			{
				...
				$this->add_attribute('organizer',stripslashes($GLOBALS['phpgw']->strip_html($this->stream->f('organizer'))));
	...
	*/
	...
	// OLD-ALARM			'phpgw_cal_alarm'
		...
		function save_event(&$event,$sendMail = true, $importAccount = false)
		{
	...
	// OLD-ALARM			'phpgw_cal_alarm'
			...
			$sql = 'UPDATE phpgw_cal SET '
					...
					. "organizer='".$this->stream->db_addslashes($event['organizer'])."', "
	...
	*/
			...
			$this->event = $event;
                        ...
                        {
                            if($importAccount)
                                $this->sendEmailEvendAccepted($event,$importAccount);
                            else
                                $this->sendEmailEvendAccepted($event);
                        }
	
			return True;
		}

                function sendEmailEvendAccepted($event,$importAccount = false)
                {

                    if(!$importAccount)
                        $importAccount['mail'] = $GLOBALS['phpgw_info']['user']['email'];

                    //verifica se eh um evento externo se sim envia o vcard de aceitação em anexo
                    if($event['owner'] == '-2')
                    {
                        $mailService = ServiceLocator::getService('mail');

                        $tmpbody.= "<b>".lang("Event Accepted")."</b> ";
                        $tmpbody.= "<br><br>".lang("name").": ".$event['title']."<br>";
                        $tmpbody.= "<br>".lang("description").": ".$event['description']."<br>";

                        $ev[0] = $event;

                        $tmpattach=  $GLOBALS['calendar']->bocalendar->create_vcard($ev,'REPLY',true,$importAccount);
                        
                        $mailService->addStringAttachment($tmpattach, 'suggestion.ics', 'application/ics');
                        $mailService->addStringAttachment($tmpattach, 'vcalendar.ics', 'utf-8', 'text/calendar; charset=utf-8; method=REPLY;', '7bit', '');
                 
                        if(strchr($event['organizer'],'@') && strchr($event['organizer'],'<') && strchr($event['organizer'],'>'))
                        {
                            $mail = substr($event['organizer'], strpos($event['organizer'],'<'), strpos($event['organizer'],'>'));
                            $mail = str_replace('<','', str_replace('>','',$mail));
                        }

                        if($mail)
                            $mailService->sendMail($mail,$importAccount['mail'],$event['title'],$tmpbody);

                    }
                    else
                        $GLOBALS['calendar']->bocalendar->send_update(MSG_ACCEPTED, $event['participants'],$event);
