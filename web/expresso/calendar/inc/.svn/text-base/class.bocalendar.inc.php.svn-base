...
//		var $debug = True;
		...
		var $xmlrpc = False;	// not called via xmlrpc
       	var $async = false;
                ...
                var $alreadyNotifieds;
                var $alreadyExtNotifieds = array();
...
//				print_debug('count event exceptions',count($event['recur_exception']));
				$this->so->add_entry($event,true,$event['owner']);
				$cd = 16;
...
//			}
		...
		function expunge()
		{
			...
			while(list($i,$event_id) = each($this->so->cal->deleted_events))
			{
				...
				{
					$this->send_update(MSG_DELETED,$event['participants'],$event,false,$event['owner']);
				}
				else
				{
		...
		function search_keywords($keywords)
		{
			...
			return $this->so->list_events_keyword($keywords,$members);
                ...
                // TODO: Levar em consideração no cálculo das ocorrências o parâmetro
		...
		{
			$prefix = ( $params['from_mobile'] ? "mobile.ui_mobile" : "calendar.ui" ) . "calendar";
			
			$ui_return = "$prefix.edit";
			$ui_index = "$prefix.index";
			...
			$l_participants = (@$params['participants']?$params['participants']:$_POST['participants']);
                        ...
                        require_once(dirname(__FILE__).'/class.ex_participants.inc.php');
                        $this->ex_participants = new exParticipants();
                        $this->ex_participants->setParticipantsByString(@$params['ex_participants']?$params['ex_participants']:$_POST['ex_participants']);

                        if(@$params['h_exParticipants']?$params['h_exParticipants']:$_POST['h_exParticipants'])
                             $this->ex_participants->updateAttribBySerializable(@$params['h_exParticipants']?$params['h_exParticipants']:$_POST['h_exParticipants']);
...
*/
			...
			else
			{
				...
				{
					$l_cal['ex_participants'] = $this->ex_participants->getParticipantsSerializable();
				}
				...
				$this->add_attribute('uid',$l_cal['uid']);
                                ...
                                $this->add_attribute('uid',time().'@Expresso');
				...
				switch((int)$l_cal['recur_type'])
				{
					case MCAL_RECUR_NONE:
						...
						$this->repetido = (int)$l_cal['recur_type']; // recebe o tipo de repeticao;
				...
				if ($l_cal['alarmdays'] > 0 || $l_cal['alarmhours'] > 0 ||$l_cal['alarmminutes'] > 0)
				{
					...
					$event['alarm'][] = Array(
						...
						'owner'   => $this->owner,
						'enabled' => 1,
						'repeat'  => $this->repetido, // para repetir alarme;
			    ...
			    $overlapping_events = $this->event_overlap( $event, $send_to_ui, $prefix );

			    if( $overlapping_events )
				return( $overlapping_events );
			}

				$event['ex_participants'] = $this->ex_participants->getParticipantsSerializable();
			
                        if($event['owner'] != $GLOBALS['phpgw_info']['user']['account_id']) $userSend = $event['owner'];
                        else $userSend = false;
       
			if(!$event['id'])
			{
                                /** Whenever a recurrent event is edit in a way that a new  
                                 * exception is created, an exception must be added to 
                                 * the old event. This is necessary to keep consistency 
                                 * with synchornization solutions in accordancy to 
                                 * the iCalendar RFC. 
                                 */ 
                                $old_event = $this->so->read_entry($_POST['cal']['reference']); 
                                ...
                                $exception_time = mktime($old_event['start']['hour'],$old_event['start']['min'],0,$month,$day,$year) - $GLOBALS['phpgw']->datetime->tz_offset; 
                                $old_event['recur_exception'][] = (int)$exception_time; 
                                $this->so->add_entry($old_event,false); 
                            
                            
                                $this->so->add_entry($event,false);
			...
			else
			{
				...
				$this->so->cal->event = $event;
				$this->so->add_entry($event,false);
				$this->prepare_recipients($new_event,$old_event,$userSend);
                                $send_type = MSG_MODIFIED;
			}
                     
                        //Retira da lista usuarios ja notificados
                        $to_notify = $event['participants'];
                        foreach ($this->alreadyNotifieds as $key => $value) 
                            if(array_key_exists($value,$to_notify))
                                unset ($to_notify[$value]);
                        //------------------------------------------------//
                           
                        if(count($to_notify) > 0 && $userSend !== -2)    
                            $this->send_update( $send_type,$to_notify,'',$this->get_cached_event(),$userSend);
		...
		function event_overlap( $event, $send_to_ui = false, $prefix = "calendar.uicalendar" ,$user = false)
		{
		    $event_ids = array();
				...
				if($event['reference'])
				{
                    ...
                    if (isset($this->repetido) and $this->repetido > 0)
                    { 
...
//							if((($temp_start_time <= $temp_event_start) && ($temp_end_time >= $temp_event_start) && ($temp_end_time <= $temp_event_end)) ||
		...
		*/
		...
		{
			if($is_private && $event['type'] == 'E')
			{
				return lang('Restrict');
			}
			else if($is_private && $event['type'] == 'P')
			{
				return lang('private');
			}
...
//				return substr($event[$field], 0 , 19) . '&nbsp;...';
				...
				else
				{
					...
					switch($type)
					{
						case MCAL_RECUR_DAILY:
							...
							break;
						case MCAL_RECUR_WEEKLY:
							if ((floor($search_beg_day / 604800) - floor($event_beg_day / 604800)) % $freq)
							{
								continue;
							}
							$check = 0;
		...
		*/
		function set_status($cal_id,$status,$user = false)
		{
			$status2msg = array(
			...
			$event = $this->so->read_entry($cal_id);
                        if($user) $account_id = $user;
                        else $account_id = $GLOBALS['phpgw_info']['user']['account_id'];
			if(($status2msg[$status] == "5" && $event['participants'][$account_id] == "A") ||
			 ($status2msg[$status] == "3" && $event['participants'][$account_id] == "R")) {
				return True;
			}
			$this->so->set_status($cal_id,$status,$account_id);
			$event = $this->so->read_entry($cal_id);
			$this->send_update($status2msg[$status],$event['participants'],$event,false,$account_id);

			$sts = array_values( $event['participants'] );

			//verifica se todos os participantes rejeitaram o evento
			while( $sts[0] && $sts[0] === 'R' ) array_pop( $sts );

			if( empty( $sts ) )
			{
				$this->so->delete_entry( $cal_id );
				$this->so->expunge();
			}
		}
		...
		*/
		...
		function create_vcard($event_arrays , $metodo = 'PUBLISH', $externo = false,$importAccount = false)
		{
                    if(!$importAccount)
                        $importAccount['mail'] = $GLOBALS['phpgw_info']['user']['email'];
		    ...
		    //$offset = ((int)substr(date('O',$GLOBALS['phpgw']->datetime->users_localtime), 0, 3));
		    
		    //Recupera o offset do cliente com base na diferenca entre o horario do cliente (timezone da preferencia)
		    //e o unixtime
		    //$offset = ((($GLOBALS['phpgw']->datetime->users_localtime) -  ($GLOBALS['phpgw']->datetime->gmtnow))/60)/60;
		    
			$timezone = date('e');

                        foreach ($event_arrays as $event_array)
                        {
                            $sy = $event_array['start']['year'];
                            $sm = $event_array['start']['month'];
                            $sd = $event_array['start']['mday'];
                            $sh = $event_array['start']['hour'];
                            $sn = $event_array['start']['min'];
                            $dtstart = sprintf("%04d%02d%02dT%02d%02d00", $sy, $sm, $sd, $sh, $sn);

                            $ey = $event_array['end']['year'];
                            $em = $event_array['end']['month'];
                            $ed = $event_array['end']['mday'];
                            $eh = $event_array['end']['hour'];
                            $en = $event_array['end']['min'];
                            $dtend = sprintf("%04d%02d%02dT%02d%02d00", $ey, $em, $ed, $eh, $en);

                            // NecessÃ¡rio espaÃ§os apÃ³s quebra-de-linha, na descriÃ§Ã£o, caso contrÃ¡rio
                            // ocorrerÃ¡ erro ao importar o agendamento no Outlook (erro lunar).
                            $description = str_replace("\n","\n ",$event_array['description']);
                            $tmpattach.="BEGIN:VEVENT\r\n";
							
			    if(isset($event_array['organizer']) && $event_array['owner'] != -2)
			    {
                                $ldap = $this->getLdap();  
                                $justthese = array("mail","cn");
                                $filter="(&(|(phpgwAccountType=u)(phpgwAccountType=l)(phpgwAccountType=s)(phpgwAccountType=i))(uidNumber=".$event_array['owner']."))";
                                $search = ldap_search($ldap, $GLOBALS['phpgw_info']['server']['ldap_context'], $filter, $justthese);
                                $entry = ldap_get_entries($ldap, $search);                                        
					$organizer = 'ORGANIZER;CN='.$entry[0]['cn'][0].':mailto:'.$entry[0]['mail'][0]."\r\n";
				}   
				else{   
					$email = explode('&lt;', $event_array['organizer']);
					$email2 = explode('&gt;', $email[1]);
                                $organizer = 'ORGANIZER;CN='.rtrim($email[0]).':mailto:'.$email2[0]."\r\n";
							}
				...
				$tmpattach.=
                                ...
                                .$organizer
                                .$this->mb_wordwrap("UID:".$event_array['uid']."\r\n",74,"\r\n ");
                                ...
                                switch ($metodo) {
                                    ...
                                    case 'REPLY':
                                            $tmpattach.=
                                             $this->mb_wordwrap("ATTENDEE;CUTYPE=INDIVIDUAL;ROLE=REQ-PARTICIPANT;PARTSTAT=ACCEPTED;X-NUM-GUESTS=0:mailto:".$importAccount['mail']."\r\n",74,"\r\n ")
                                            .'DESCRIPTION:\n'."\r\n"
                ...
                function mb_wordwrap($str, $width=74, $break="\r\n")
                {
                    ...
                    if(empty($str) || mb_strlen($str, 'ISSO-8859-1') <= $width)
                        return $str;

                    $br_width  = mb_strlen($break, 'ISSO-8859-1');
                    $str_width = mb_strlen($str, 'ISSO-8859-1');
                    $return = '';
                    $last_space = false;
                    ...
                    {
                        // If we're at a break
                        if (mb_substr($str, $i, $br_width, 'ISSO-8859-1') == $break)
                        {
                            $count = 0;
                            $return .= mb_substr($str, $i, $br_width, 'ISSO-8859-1');
                            $i += $br_width - 1;
                            continue;
                        }

                        // Keep a track of the most recent possible break point
                        if(mb_substr($str, $i, 1, 'ISSO-8859-1') == " ")
                        {
                            $last_space = $i;
                        }
                        ...
                        $return .= mb_substr($str, $i, 1, 'ISSO-8859-1');
                    }
                    return $return;
                }

                function getvCalendarParticipants($pExtParticipants,$pCalId)
                                {
                    $participants = explode(',', $pExtParticipants);
                    $return = '';

                    foreach ($participants as $participant)
                                      {
                        $array = explode('"', $participant);
                        $mail = str_replace('<','', str_replace('>','',$array['2']));
                        if($mail && $array['1'])
                             $return .= 'ATTENDEE;CN='.$array['1'].';RSVP=TRUE:mailto:'.$mail."\r\n";
                    }
                    $db = $this->getDb();
                    $db->query('SELECT cal_login FROM phpgw_cal_user WHERE cal_id = '.$pCalId);
                    $parts = array();
                    while($db->next_record())
                        array_push($parts, $db->row());

                    $ldap = $this->getLdap();
                    foreach ($parts as $part)
                    {
                        $justthese = array('mail','cn');
                        $filter="(&(|(phpgwAccountType=u)(phpgwAccountType=l))(uidNumber=".$part['cal_login']."))";
                        $search = ldap_search($ldap, $GLOBALS['phpgw_info']['server']['ldap_context'], $filter, $justthese);
                        $entry = ldap_get_entries($ldap, $search);
                        $return .= 'ATTENDEE;CN='.$entry[0]['cn'][0].';RSVP=TRUE:mailto:'.$entry[0]['mail'][0]."\r\n";
                                                        }

                    return $return;
                                                        }
				
                function getCalendarParticipantsArray($pCalId)
                {
                    $db = $this->getDb();
                    $db->query('SELECT cal_login FROM phpgw_cal_user WHERE cal_id = '.$pCalId);
                    $parts = array();
                    ...
                    foreach ($parts as $part)
                    {
                        ...
                        $search = ldap_search($ldap, $GLOBALS['phpgw_info']['server']['ldap_context'], $filter, $justthese);
                        $entry = ldap_get_entries($ldap, $search);
                        $return [$entry[0]['mail'][0]] = array( 'CN'=> $entry[0]['cn'][0],'RSVP'=> 'TRUE' ,'ROLE' => 'REQ-PARTICIPANT');
                    }
                    return $return;
                }

                function getvCalendarExternalParticipants($pCalId)
                {

                    $db = $this->getDb();
                    $sql = "SELECT ex_participants FROM phpgw_cal WHERE cal_id = '$pCalId'";
                                $db->query($sql);
					...
					$participants = explode(',', $parts[0]['ex_participants']);
                    $return = '';
					
					$participants = unserialize(base64_decode($parts[0]['ex_participants']));

                    foreach ($participants as $participant)
                    {	
                           $part = array();

                           if(preg_match("(\"[^\"]*\"[ ]*<[^>]*@[^>]*>)", $participant['mail']))
                           {
                              $cn = null;
                              $mail = null;
                              preg_match ("(\"([^\"]*)\")", $participant['mail'], $cn);
                              preg_match ('(<([^>]*@[^>]*)>)', $participant['mail'], $mail);
                          ...
                          {
                              if($part['cn']) 
								$return .= 'ATTENDEE;CN='.$part['cn'].';RSVP=TRUE:mailto:'.$part['mail']."\r\n";
                              else 
								$return .= 'ATTENDEE;RSVP=TRUE:mailto:'.$part['mail']."\r\n";  
                ...
                function getLdap()
                {
                    ...
                    require_once(dirname(__FILE__).'/../../phpgwapi/inc/class.common.inc.php');
                ...
                function getvCalendarOrganizer($pOrganizer)
                {

                    
                       $organizer = array();
       
                       if(preg_match("(\"[^\"]*\"[ ]*<[^>]*@[^>]*>)", $pOrganizer))
                       {
                          $cn = null;
                          $mail = null;
                          preg_match ("(\"([^\"]*)\")", $pOrganizer, $cn);
                          preg_match ('(<([^>]*@[^>]*)>)', $pOrganizer, $mail);

                          if($mail[1])
                          $organizer['mail'] =  $mail[1];
                       ...
                       if(preg_match("([^ ]*@[^ ]*)", $pOrganizer))
                       {
                          ...
                          preg_match ('([^ ]*@[^ ]*)', $pOrganizer, $mail);
                          if($mail[0])
                            $organizer['mail'] =  $mail[0];

                       }
                       
                       if($organizer['cn']) $return = 'ORGANIZER;CN="'.$organizer['cn'].'":MAILTO:'.$organizer['mail']."\r\n";
		...
		function send_update($msg_type,$to_notify,$old_event,$new_event = False, $user = False)
			{
                        ...
                        if (!is_array($to_notify)) $to_notify = array();
			...
			$GLOBALS['phpgw_info']['user']['preferences'] = $GLOBALS['phpgw']->preferences->create_email_preferences();
                        ...
                        /*
                         * Caso o evento seja de uma conta compartilha busca obusca o cn e o email da conta
                         */ 
                        if($user)
                        {
                            $ldap = $this->getLdap();  
                            $justthese = array("mail","cn");
                            $filter="(&(|(phpgwAccountType=u)(phpgwAccountType=l)(phpgwAccountType=s)(phpgwAccountType=i))(uidNumber=".$user."))";
                            $search = ldap_search($ldap, $GLOBALS['phpgw_info']['server']['ldap_context'], $filter, $justthese);
                            $entry = ldap_get_entries($ldap, $search);
                            $sMail = $entry[0]['mail'][0];
                            $sCN = $entry[0]['cn'][0];
                            $sUN = $user;
 		        } 
                        else
                        {
                            $sMail = $GLOBALS['phpgw_info']['user']['email'];
                            $sCN = $GLOBALS['phpgw_info']['user']['fullname'];
                            $sUN = $GLOBALS['phpgw_info']['user']['account_id'];
                        }
                        //------------------------------------------------------------------------------------//
			...
			$GLOBALS['phpgw_info']['user']['preferences'] = $GLOBALS['phpgw']->preferences->create_email_preferences($user);
                        $this->prefs = $GLOBALS['phpgw_info']['user']['preferences'];
			...
			foreach($to_notify as $userid => $statusid) 
			{
				...
				if ($statusid == 'R' || $GLOBALS['phpgw']->accounts->get_type($userid) == 'g' || ($sUN == $userid &&  $msg_type != MSG_ALARM))
                                    continue;	// dont notify rejected participants or to == send
				
				if($userid != $sUN  ||  $msg_type == MSG_ALARM)
				{
					...
					if( $userid === -2 )
					{
						$to = htmlspecialchars_decode( $event['organizer'] );
					}
					...
					switch($part_prefs['calendar']['update_format'])
 					{
						case  'extended':
										...
										foreach($newvalue as $tmp)
										{
											...
											if( trim($tmpmail) == trim($to) & trim($tmpuid) == trim($NewKey))
											{
													if($typesend == 3)
													{
														...
														$lang5 = lang("Suggest new hour"); 
														$varbuttom4 = "<form action=".$GLOBALS['phpgw_info']['server']['webserver_url']."/index.php?menuaction=calendar.uicalendar.view&cal_id=$event[id]&date=$newall3&type=1' method='POST'>
							...
							if(!PHPGW_APP_TPL)
                                                         define ("PHPGW_APP_TPL",PHPGW_INCLUDE_ROOT . "/calendar/templates/".$GLOBALS['phpgw_info']['user']['preferences']['common']['template_set']."");
							
								$body = CreateObject('phpgwapi.Template',PHPGW_INCLUDE_ROOT . "/calendar/templates/".$GLOBALS['phpgw_info']['user']['preferences']['common']['template_set']."");
...
*/
					...
					unset($vardata6);
                        ...
                        if(!is_object($this->ex_participants))
                        {
                            include_once(dirname(__FILE__).'/class.ex_participants.inc.php');
                            $objexP = new exParticipants();
                            $objexP->setParticipantsBySerializable($this->ex_participants);
				}
				else
                            $objexP = $this->ex_participants;

			
                        if($msg_type === MSG_ADDED && count($this->extAdded) > 0)
                               $extMails = $this->extAdded;  
                        else if($msg_type === MSG_DELETED && count($this->extDeleted) > 0)
                                $extMails = $this->extDeleted;  
                        else 
                            $extMails = array_diff($objexP->getParticipantsMailsArray(), $this->alreadyExtNotifieds);
			...
			if(count($extMails) > 0){
				...
				foreach($extMails as $index => $ex_participant){
				...
				$tmpbody.= "<br><b>".lang("external participants").":: </b> ".htmlentities($objexP->getParticipantsMailsString());
				$tmpbody.= "<br>".lang("Summary").": ".$this->so->cal->event[description]."<br>";
				...
				.lang("This message was sent by server. You must send a message to sender to confirm this event")."<br>"
				.lang("This is an external event. Even if it added to your expresso its can be changed any time at all")."</font><br>";
				...
				$evt = $GLOBALS['bocalendar']->so->cal->event;
                                ...
                                if($msg_type == MSG_DELETED)
                                {
                                        $icalService->createICal(strtoupper($method));
                                        $icalService->addEvent(
                                            $old_event['start']['year'].'-'.$old_event['start']['month'].'-'.$old_event['start']['mday'].' '.$old_event['start']['hour'].':'.$old_event['start']['min'].':00',
                                            $old_event['end']['year'].'-'.$old_event['end']['month'].'-'.$old_event['end']['mday'].' '.$old_event['end']['hour'].':'.$old_event['end']['min'].':00',
                                            array($sMail => array('CN'=> $sCN)),
                                            $old_event['title'],
                                ...
                                else
                                {
                                        $attendee = array();
                                        foreach($to as $value)
                                            $attendee[$value] = array('RSVP' => 'TRUE','ROLE' => 'REQ-PARTICIPANT');

                                        $attendee =  array_merge($attendee,$this->getCalendarParticipantsArray($evt['id']));
                                        $icalService->createICal(strtoupper($method));
                                        $icalService->addEvent(
                                            $evt['start']['year'].'-'.$evt['start']['month'].'-'.$evt['start']['mday'].' '.$evt['start']['hour'].':'.$evt['start']['min'].':00',
                                            $evt['end']['year'].'-'.$evt['end']['month'].'-'.$evt['end']['mday'].' '.$evt['end']['hour'].':'.$evt['end']['min'].':00',
                                            array($sMail => array('CN'=> $sCN)),
                                            $evt['title'],
                                            $evt['description'],
                                            $evt['location'],
				...
				$mail->Sender = $mail->From;
				$mail->SenderName = $mail->FromName;
				$mail->Subject = lang("External event from Expresso");
				$mail->Body = $tmpbody;
			...
			// Notifica por email o criador do compromisso, com as poss?veis falhas.						
			if($errorInfo) {
		 ...
		 */
		function send_suggestion_external_owner($cal_id, $uid, $to, $time_suggest, $array_data_inicio, $array_data_final, $title, $description, $location, $ex_participants,$user = false)
                {
                    ...
                    $this->so->cal->set_status($event['id'],$user,2);


                    if($user && $user != $GLOBALS['phpgw_info']['user']['account_id'])
                    {
                        $ldap = $this->getLdap();
                        $justthese = array("mail","cn");
                        $filter="(&(|(phpgwAccountType=u)(phpgwAccountType=l)(phpgwAccountType=s)(phpgwAccountType=i))(uidNumber=".$user."))";
                        $search = ldap_search($ldap, $GLOBALS['phpgw_info']['server']['ldap_context'], $filter, $justthese);
                        $entry = ldap_get_entries($ldap, $search);
                        $ownerMail = $entry[0]['mail'][0];
                        $ownerCN = $entry[0]['cn'][0];
                     }
                     else
                     {
                        $ownerMail = $GLOBALS['phpgw_info']['user']['email'];
                        $ownerCN = $GLOBALS['phpgw_info']['user']['fullname'];
                     }
                     
                    require_once dirname(__FILE__) . '/../../services/class.servicelocator.php';
                    $icalService = ServiceLocator::getService('ical');
                    $dia_inicio = $array_data_inicio[0];
                    $mes_inicio = $array_data_inicio[1];
                    $ano_inicio = $array_data_inicio[2];
                    $hora_inicio = $time_suggest['hora_inicio'];
                    $minuto_inicio = $time_suggest['minuto_inicio'];
                    $dia_final = $array_data_final[0];
                    $mes_final = $array_data_final[1];
                    $ano_final = $array_data_final[2];
                    $hora_final = $time_suggest['hora_final'];
                    $minuto_final = $time_suggest['minuto_final'];

                    $dtStart = $array_data_inicio[2] . '/' .
                               $array_data_inicio[1] . '/' .
                               $array_data_inicio[0] . ' ' .
                               $time_suggest['hora_inicio'] . ':' .
                               $time_suggest['minuto_inicio'] . ':00';

                    $dtEnd = $array_data_final[2] . '/' .
                             $array_data_final[1] . '/' .
                             $array_data_final[0] . ' ' .
                             $time_suggest['hora_final'] . ':' .
                             $time_suggest['minuto_final'] . ':00';

                    $icalService->createICal('REPLY');

                    $other = array( 'X-MICROSOFT-CDO-IMPORTANCE' => '1',
                                    'X-EXPRESSO-SUGGESTION-HOUR' => '1',
                                    'CLASS' => 'PUBLIC',
                                    'PRIORITY' => '5',
                                    'SEQIENCE' => '0',
                                    'TRANSP' => 'OPAQUE');
                    ...
                    /* Corpo do email */
                    $tmpbody2 = lang("The sender suggested a new hours for the appointment");
                    $tmpbody2.= " <br /> <b>" . lang("Title") . ":</b> $title";
                    $tmpbody2.= " <br /> <b>" . lang("Details") . ":</b> <br /> <br /> <b>" . lang("Start") . "  </b>" . $dia_inicio . "/" . $mes_inicio . "/" . $ano_inicio . " - " . $hora_inicio . ":" . $minuto_inicio;
                    $tmpbody2.= " <br /> <b>" . lang("End") . " </b>" . $dia_final . "/" . $mes_final . "/" . $ano_final . " - " . $hora_final . ":" . $minuto_final . "<br><br><br><br>";
                    $tmpbody = lang("The sender suggested a new hours for the appointment");
                    $tmpbody.= " \r\n " . lang("Title") . ": $title";
                    ...
                    $icalService->addEvent($dtStart,
                            $dtEnd,
                            $to,
                            $title,
                            utf8_encode($tmpbody),
                            $location,
                            array($ownerMail => array('CN' => $ownerCN, 'PARTSTAT' => 'TENTATIVE')),
                            $other,
                            false,
                            $uid
                    );
                    ...
                    $mailService->addStringAttachment($tmpattach, 'suggestion.ics', 'application/ics');
                    $mailService->addStringAttachment($tmpattach, 'vcalendar.ics', 'text/calendar; charset=utf-8; method=REPLY;', '7bit', 'inline');
                    $mailService->sendMail($to, $ownerMail, $title, $tmpbody2);
		...
		function prepare_recipients(&$new_event,$old_event,$userSend)
		{
                        //Verifica se ouve alguem deletado
                        foreach($old_event['participants'] as $key => $value)
                           if(!array_key_exists($key, $new_event['participants']))
				{
                               $this->deleted[$key] = $value;
                               $this->alreadyNotifieds[] = $key; //Adiciona a variavel de ja notificados para não serem notificados 2 vezes;
				}
                        //Verifica se ouve alguem adicionado
                        foreach($new_event['participants'] as $key => $value)
                           if(!array_key_exists($key, $old_event['participants']))
				{
                               $this->added[$key] = $value;
                               $this->alreadyNotifieds[] = $key; //Adiciona a variavel de ja notificados para não serem notificados 2 vezes;
			}
			
                        //Verifica se ouve alguem modificado
                        foreach($new_event['participants'] as $key => $value)
			{
                            if(array_key_exists($key, $old_event['participants']))                       
				{
                                if($old_event['participants'][$key] != $value)
				{
                         ...
                         //Verificando alterações em usuarios externos
                         include_once(dirname(__FILE__).'/class.ex_participants.inc.php');
                         $extOld = new exParticipants();
                         $extNew = new exParticipants();
                         
                         $extOld->setParticipantsBySerializable($old_event['ex_participants']);
                         $extNew->setParticipantsBySerializable($new_event['ex_participants']);
                        
                         $oldArray = $extOld->getParticipantsMailsArray();
                         $newArray = $extNew->getParticipantsMailsArray();
                         
                        //Verifica se ouve alguem deletado
                        $this->extDeleted = array_diff($oldArray, $newArray);
                        
                        //Verifica se ouve alguem adicionado
                        $this->extAdded = array_diff($newArray, $oldArray);
                        
                        $this->alreadyExtNotifieds = array_merge($this->alreadyExtNotifieds, $this->extDeleted);
                        $this->alreadyExtNotifieds = array_merge($this->alreadyExtNotifieds, $this->extAdded);
                        //---------------------------------------------------------------------------------//
                        
                        if(count($this->added) > 0 || count($this->extAdded))
                                $this->send_update(MSG_ADDED,$this->added,'',$new_event,$userSend);
                        
                        if(count($this->modified) > 0)
                                $this->send_update(MSG_MODIFIED,$this->modified,$old_event,$new_event,$userSend);
                        
                        if(count($this->deleted) > 0 || count($this->extDeleted))
                                $this->send_update(MSG_DELETED,$this->deleted,$old_event,false,$userSend);
		...
		*/
		function event2array($event)
		{
		  ...
		  $var['title'] = Array(
			...
			$owner_name = ( $owner === -2 )? $event['organizer'] : $GLOBALS['phpgw']->common->grab_owner_name( $owner );
			...
			if($event['public']){
				$type = lang('Public');
			}else if (!$event['public'] && $event['type'] == 'E'){
				$type = lang('Restrict');
			}else{
				$type = lang('Private');
			}
