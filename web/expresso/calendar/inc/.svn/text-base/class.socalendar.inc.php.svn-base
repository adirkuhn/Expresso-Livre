...
//			$sql .= 'AND (((phpgw_cal_repeats.recur_enddate >= '.$starttime.') AND (phpgw_cal_repeats.recur_enddate <= '.$endtime.')) OR (phpgw_cal_repeats.recur_enddate=0))) '
		...
		{
			$uid_ = substr($uid, strpos($uid,"-")+1);				
			$uid_ = substr($uid_, 0, strpos($uid_,"-"));				
			
			$sql = " AND (phpgw_cal.cal_id = '". (int)$uid_."') ";
		...
		/*
		 * @abstract: Retorna o email do organizer do evento. 
		 */
		function get_mail_organizer($cal_id)
		{
			$sql = 'SELECT organizer FROM phpgw_cal WHERE cal_id =' . $cal_id;	
			$this->cal->stream->query($sql,__LINE__,__FILE__);
			
			while($this->cal->stream->next_record())
			{
				$ret[0] = $this->cal->stream->f('organizer');
			}
		...
		/*
		 * @abstract: Retorna os emails dos participantes do evento.
		 */
		function get_mail_participants($cal_id)
		{
		...
		/*
		 * @abstract: busca no banco pra verificar se já existe um compromisso com o mesmo cal_id
		 */
		function find_cal_id($cal_id)
		{

                        $uid_ = substr($cal_id, strpos($cal_id,"-")+1);
                        $uid_ = substr($uid_, 0, strpos($uid_,"-"));

                        $sql = " AND phpgw_cal.cal_id = '". (int)$uid_."' OR phpgw_cal.uid = '". $cal_id."' ";
		...
		function add_entry(&$event , $sendMail = true, $importAccount = false)
		{
			return $this->cal->save_event($event,$sendMail,$importAccount);
		}
		...
		function set_status($id,$status,$user = false)
		{
                    if($user) $this->cal->set_status($id,$user,$status);
                    else $this->cal->set_status($id,$this->owner,$status);
		}
