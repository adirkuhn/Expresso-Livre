...
// Funcoes	
        ...
        function sChangeHour(pSelect)
        {
            if(pSelect.selectedIndex < 1)
                return;
            
            var hour = pSelect.options[pSelect.selectedIndex].value;                         
            var sHour = hour.split(':');           
            
            var hHour,mHour;
            if(pSelect.name == 'sEndHour')
            {
                hHour = document.getElementById('end_hour');
                mHour = document.getElementById('end_minute');
            }
            else if(pSelect.name == 'sStartHour')
            {
                hHour = document.getElementById('start_hour');
                mHour = document.getElementById('start_minute');
            }    
    ...
    function show_disponibility() { 
    	...
    	participants = ""; 
		    ...
		    if(combo.length==0) { 
		    	var msg_ = document.getElementById("set_participants").value;
				alert(msg_);
		        return; 
		    } 
	...
	function openListUsers(newWidth,newHeight, owner){					
		...
		newScreenY  = 0;		
		window.open('calendar/templates/default/listUsers.php?owner='+owner,"","width="+newWidth+",height="+newHeight+",screenX="+newScreenX+",left="+newScreenX+",screenY="+newScreenY+",top="+newScreenY+",toolbar=no,scrollbars=yes,resizable=no");
	}
