...
function optionFinderLdap(id,numMin, type){
		...
		if (oText.value.length >= numMin){
			var context = document.getElementById('combo_org').value;
			cExecute ('expressoMail1_2.ldap_functions.search_users&context='+(context)+'&type='+(type == '' ? 'list' : 'search')+'&filter='+oText.value, handler_get_available_users);
			oWait.innerHTML = '&nbsp;';
		}
}
...
function showExParticipants(el,path){
	...
	loadScript("calendar/templates/classic/js/over_fn_exmail.js");
	if(!contacts)
