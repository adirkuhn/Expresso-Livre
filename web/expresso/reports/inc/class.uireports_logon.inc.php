	...
	*************************************************************************************/
	...
	class uireports_logon
	{
		...
		function report_logon_print_pdf()
		{
			...
			$pdf->Open();
			$pdf->AddPage();
			$pdf->SetTitle('Relatório Gerado pelo Expresso Reports');
			...
			$SubTituloR = lang('report of time without logging by Organization');
...
//			$pdf->Cell(0,10,$SubTitulo1,0,1,'C',0);
			...
			{ 
				...
				while (list($null,$accountr) = each($returnp))
				{
						...
						$row_mail = (!$accountr['account_mail'] ? 'Sem E-mail' : $accountr['account_mail']);
						...
						$row_li_date = $accountr['li_date'] == 'Nunca logou' ? 'Nunca logou' : $accountr['li_date'];
						$row_li_dias = $accountr['li_dias'];
						$row_status = $accountr['account_status'] == 'active' ? 'Ativado' : 'Desativado';
				...
				if ($vnumacesso==0){
					...
					while (list($null,$accountr) = each($returnp))
					{
						...
						$varr = array(
							...
							'row_timestamp'		=> substr($accountr['createTimestamp'], 6, 2)."/".substr($accountr['createTimestamp'], 4, 2)."/".substr($accountr['createTimestamp'], 0, 4),
							'row_li_date'		=> $accountr['li_date'] =='Nunca logou' ? '<font color="#FF0000">'.$accountr['li_date'].'</font>' : $accountr['li_date'],
							'row_li_dias'		=> $accountr['li_dias']
						);
