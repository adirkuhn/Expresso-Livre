...
class ldap_functions
{
	...
	function ldapRootConnect($refer = false){
		...
		if($_SESSION['phpgw_info']['expressomail']['server']['ldap_master_host'] &&
	 ...
	 /**
        ...
        */
        ...
        {              
			...
			foreach($info_return as $i=>$value)
				$info_return[$i]['type_contact'] = "G";
				
			$info_return['search_for'] = $params['search_for'];
            return $this->toISO8859($info_return); 
        } 
	...
	function getSharedUsersFrom($params){
		...
		if($params['uids']) {
			...
			foreach($uids as $index => $uid){		
				...
				$user_arr = array();
				$user_arr['user'] = $uid;
				$user_arr['getShared'] = true;
				$acl = $this->imap->getaclfromuser( $user_arr );
				
				if($acl[$uid] != 'false'){
					if ( preg_match("/[pa]+.*[pa]+/",$acl[$uid])){	
						$filter .= "(uid=$uid)";							
						$acl_save_sent_in_shared[] =$uid;			
					}	
				}
			}			
		}
