...
class imap_functions
{
	...
	*/
		...
		include_once dirname(__FILE__).'/../../prototype/api/controller.php';
            ...
            if(!$search_box_type || $search_box_type == 'UNSEEN' || $search_box_type == 'SEEN') {
                    ...
                    if(is_array($sort_array_msg)){
							...
							for($i=0; $i<count($sort_array_msg); $i++){
	...
	{             
            ...
            $return['flag'] = $header->Unseen.$header->Recent.$header->Flagged.$header->Draft.$header->Answered.$header->Deleted.( $return['attachment'] === '1' ? 'T': '' );
	...
	*/
	function get_msg_sample($msg_number)
	{
		...
		$content != "" ? $return['body'] = " - " . $content: $return['body'] = "";
                $return['body'] = base64_encode(mb_convert_encoding(substr($return['body'], 0, 305),'ISO-8859-1' , 'UTF-8,ISO-8859-1'));
		return $return;
	}
	...
	function get_info_msg($params)
	{
		...
		$return['msg_folder'] = isset($params['decoded']) ? mb_convert_encoding( $msg_folder, "ISO-8859-1", "UTF-8" ) : $msg_folder;
...
//            $msg->fetch_structure($msg_number);
	...
	*/			
	...
	function replace_links_callback($matches)  
	{
 	    ...
 	    return '<a href="'.$pref.'://'.$matches[4].$matches[5].'" target="_blank">'.$matches[0].'</a>'; 
 	} 
	...
	*/
	...
	function getFolders($params = null)
	{
		...
		foreach ($folders_list as $i => $v ) //Separando Pastas e informações
		{
			...
			if($nameArray[0] === 'user')
				$folders[$prefixShared.$this->imap_delimiter.$nameArray[1]][] = array('id' => $folderId , 'stream' => $v->name , 'attributes' => $v->attributes , 'name' => $nameArray[($nameCount-1)] , 'user' => $nameArray[1] ,'parent' => $parent);
			else if( $folderId !== $decifrada) //Escapa pasta decifrada
				$folders['INBOX'][strtolower($folderId)] =  array('id' => $folderId , 'stream' => $v->name , 'attributes' => $v->attributes ,'name' => $nameArray[($nameCount-1)] , 'parent' => $parent);
		}
    ...
    //	    TODO - implementar tratamento SMIME no novo serviço de envio de emails e retirar o AND false abaixo
            if ($params['smime'] AND false) {
                ...
                $body = mb_ereg_replace('<!--\[', '<!-- [', $params['body']);
    ...
    //		TODO - tratar assinatura e remover o AND false
            ...
            if ($fromaddress)
                $mailService->setFrom('"' . $fromaddress[0] . '" <' . $fromaddress[1] . '>');
            else
                $mailService->setFrom('"' . $_SESSION['phpgw_info']['expressomail']['user']['firstname'] . ' ' . $_SESSION['phpgw_info']['expressomail']['user']['lastname'] . '" <' . $_SESSION['phpgw_info']['expressomail']['user']['email'] . '>');
                ...
                else //message attachment
                {
                    ...
                    switch ($value['type']) {
                        case 'imapPart':
                                ...
                                if(strstr($body,'<img src="./inc/get_archive.php?msgFolder='.$value['folder'].'&msgNumber='.$value['uid'].'&indexPart='.$value['part'].'" />') !== false)//Embeded IMG
                                {    
                                    $body = str_ireplace('<img src="./inc/get_archive.php?msgFolder='.$value['folder'].'&msgNumber='.$value['uid'].'&indexPart='.$value['part'].'" />' , '<img src="'.$att['name'].'" />', $body);
	...
	function set_messages_flag_from_search($params){		
		...
		if (count($sorted_msgs)==1){
			...
			for($i = 0; $i < count($array_names_keys); $i++){ 
	...
	{
		$folder = 'INBOX' . $this->imap_delimiter . $_SESSION['phpgw_info']['expressomail']['email_server'][$params['clean_folder']];
		$mbox_stream = $this->open_mbox($folder);
...
// 	Fix the search problem with special characters!!!!
	...
	function getaclfromuser($params)
	{
		...
		$mbox_stream = $this->open_mbox();
		if($params['getShared']){		
			 $mbox_acl = imap_getacl($mbox_stream, 'user'.$this->imap_delimiter.$useracl); 		
			foreach ($mbox_acl as $user => $acl)
			{
	...
	function getacltouser($user, $decode = false)
	{
		...
		$mbox_acl = imap_getacl($mbox_stream, 'user'.$this->imap_delimiter.$user);
		  ...
		  $mbox_acl = @imap_getacl($mbox_stream, $user);
                ...
                if(isset($mbox_acl[$this->username]))
...
//Por Bruno Costa(bruno.vieira-costa@serpro.gov.br - Recebe os fontes dos emails a serem desarquivados, separa e envia cada um para função insert_mail.
    ...
    {		
		...
		$timestampsactual = $timestamps[$index] + $this->functions->CalculateDateOffset(); 
 		        $insert = $this->insert_email($source, mb_convert_encoding( $dest_folder,"ISO-8859-1","UTF-8"), $timestampsactual,$flags[$index]); 
            }
	}
        return $insert;
    }
	...
	function quickSearchMail( $params )
	{
		...
		if($params['sort'] !== 'SORTDATE_REVERSE')
		if(strpos($params['sort'],'REVERSE') !== false)
	...
	function searchSieveRule( $params )
	{
		...
		foreach ($msgs as $i => $v)
		{
			...
			{			
				$this->mbox = $this->open_mbox( 'INBOX' );
				$b = $this->get_body_msg( $v , 'INBOX' );
				
				if( $this->filterCheck( $b['body'] , $params['body']['criteria'] , $params['body']['filter'] ))
					$rr['body'][] = $v;
		...
		{			
			if(count($rrr) == 0 && $init === true)
				$rrr = $v;
			else if($params['isExact'] === true)
				$rrr = array_diff($rrr , $v);
			else
