...
//		cExecute('$this.rss.getChannels',handlerChannel);
		...
		if (preferences.use_local_messages==1 || expresso_offline) {
			    //verifica se gears esta installado ou o offline
			if( (!window.google || !google.gears) &&
			     !expresso_local_messages.installGears() ){
                                preferences.use_local_messages = 0;
			}
			else if( !expresso_local_messages.is_offline_installed() )
				preferences.use_local_messages = 0;
			else{
				if (expresso_offline)
...
function draw_tree_labels(){
	...
	myLabels.html("<div class='my-labels'>" +
				"<img src='../prototype/modules/mail/img/tag.png'/>" +
				"<a class='title-my-labels' tabindex='0' role='button' aria-expanded='false' title='"+get_lang("My Labels")+"'>"+get_lang("My Labels")+"</a>" +
...
function changeCursorState(ev) {
	...
	if (tbl.emptyBody)
		return;
	if ((((cellWidth-cellMouseX)<15)||((el.style.cursor=="col-resize")&&(detectLeftButton())))&&((el.className=="th_resizable")&&(el.cellIndex+1<el.parentNode.cells.length))){
...
function draw_box(headers_msgs, msg_folder, alternate){
	...
	var scripts = new Array("InfoContact",
							"TreeShow");
...
function make_tr_message(headers_msgs, msg_folder, offsetToGMT){
		...
		if (headers_msgs.attachment == 1)
			td_element2.innerHTML = "<img src ='templates/"+template+"/images/clip.gif' >";
		...
		{
			td_element4.innerHTML += headers_msgs.subject + "<span style=\"color:#b3b3b3;\">  " + tr_element.msg_sample + "</span>";//modificacao feita para exibir o trecho do body ao lado do assunto da mensagem;
		}
		...
		if (headers_msgs.labels) {
			td_element27.innerHTML = '<img src="../prototype/modules/mail/img/tag.png">';
			...
			*/
			...
			if (!flagged) {
				...
				DataLayer.commit(false, false, function(data){
					...
					$('#td_message_followup_' + messageClickedId + ', ' + 
					'tr[role="'+messageClickedId+'_'+msg_folder+'"] #td_message_followup_search_' + messageClickedId).find(".flag-edited")
					.html('<img src="../prototype/modules/mail/img/flagEditor.png">');
...
function draw_message(info_msg, ID){
	...
	options.appendChild(option_reply);
		...
		option_delete.onclick = function(){proxy_mensagens.delete_msgs('null','selected','null');};
	...
	var div_other_options = document.createElement("DIV");
		...
		option_reply_to_all__title = (preferences.use_shortcuts == '1') ? get_lang('Shortcut: %1', 'T') : '';
		var option_reply_to_all = '<span onmouseover="this.className=\'reply_options_active\'" onmouseout="this.className=\'reply_options\'" class="reply_options" onclick="new_message(\'reply_to_all_with_history\',\''+ID+'\');" title="'+option_reply_to_all__title+'">'+get_lang("Reply to all")+'</span> | ';
	var option_reply_without_history = '<span onmouseover="this.className=\'reply_options_active\'" onmouseout="this.className=\'reply_options\'" class="reply_options" onclick=new_message("reply_without_history","'+ID+'");>'+get_lang("Reply without history")+'</span> | ';
		...
		div_other_more_options.innerHTML += option_move + option_print + option_export + block_user +  report_error;
		
		div_other_options.innerHTML += option_reply_without_history + option_reply_to_all_without_history + option_reply_to_all;
	if (use_spam_filter) {
	...
	if(defaultCalendar == "expressoCalendar" && $("#expressoCalendarid")[0]){
		...
		import_implements_calendar();
			...
			eventDetails({ 
					...
					isAttendee: true,
					attendee: '',//TODOOOOOO,
					calendar: User.preferences.defaultCalendar ? User.preferences.defaultCalendar : Calendar.calendars[0],
	...
	if(!info_msg.showImg && imgTag)
	{
		...
		if (preferences.notification_domains != null && typeof(preferences.notification_domains) != 'undefined')
		{
			...
			for(var j = 0; j < imgTag.length; j++)
			{
				...
				if (forbidden)
				{
					newBody = newBody.replace(imgTag[j],"<img src='templates/"+template+"/images/forbidden.png'>");
					blocked=true;
				}
			}
			if (blocked)
...
function input_keydowns(input, ID){
	...
	.focusout(function(e){
		...
		/*
		 ** Verifica se o dynamic contact esta aberto ou fechado, caso esteja aberto, 
		 ** demora 250 millisegundos para executar a função makeBoxMail.
		 */
		these.data('is_open') ? setTimeout(makeBoxMail,250) : makeBoxMail();
	})
		...
		.data( "catcomplete" )._renderItem = function( ul, item ) {
			if($(ul).find("li").length < 50){
				...
				return $(listContacts).data( "item.autocomplete", item ).appendTo( ul ).find("span:last").button({
					...
					canMakeBox = false;
					$.Zebra_Dialog('Deseja remover <b>'+(item.name ? item.name+" - " : "")+ item.mail+'</b>?', {
							'type':     'question',
...
function draw_new_message(border_ID){
	...
	content.find(".new-msg-head-right-buttons").find(".button").button().filter(".rich-button").click(function(){
		...
		change_text($(this).find(".ui-button-text"), get_lang("Rich Text"), get_lang("Simple Text"));
	})
		...
		add: function (e, data) {
			...
			$.each(data.files, function (index, file) {
				...
				if(file.name.length > 50)
					attach.fileName = file.name.substr(0, 32) + " ... " + file.name.substr(file.name.length-9, file.name.length);
			...
			if(!!data.result && data.result != "[]" ){
				...
				fileUploadMSG.find(' .progress.on-complete:first').parents('p').find('.status-upload').append('Erro ao fazer upload!').addClass('message-attach-error');
			}
				...
				jQuery.each(selectedMessages, function(folder_name, messages) {
					jQuery.each(selectedMessages[folder_name], function(message_number, message) { 
						if (message) {
							...
							var upload = $(DataLayer.render("../prototype/modules/mail/templates/attachment_add_itemlist.ejs", {file : attach}));
							upload.find('.status-upload').addClass('ui-icon ui-icon-check');
							...
							upload.find('.button.close').button({
								icons: {
										...
										var idAttach = $(this).parents("li:first").find('input[name="fileId[]"]').val();
...
//		'<span class="message_options_print"><span ' + change_font_color + ' title="'+get_lang("Print")+'" class="message_options" onclick=print_all()>'+get_lang("Print")+'</span></span>'+
	...
	if(preferences.use_local_messages==1 && !expresso_offline)
		if(proxy_mensagens.is_local_folder(current_folder))
			span_options.innerHTML += '&nbsp; <span id="localOption" ' + change_font_color + ' title="'+get_lang("Unarchive")+'" class="message_options" onclick="expresso_local_messages.unarchive_msgs(\''+folder+'\',null)">'+get_lang("Unarchive")+'</span>';
		else
			span_options.innerHTML += '&nbsp; <span id="localOption" ' + change_font_color + ' title="'+get_lang("Archive")+'" class="message_options" onclick="archive_msgs(\''+folder+'\',null)">'+get_lang("Archive")+'</span>';
	if (use_spam_filter) {
