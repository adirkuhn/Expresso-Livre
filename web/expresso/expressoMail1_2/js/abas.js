...
function delete_border(ID, msg_sent)
{
	...
	if($("#border_id_"+ID).attr("title") == get_lang("New Message")){
		var to = $("#content_id_"+ID).find(".to").parent(".email-area").find("div").length || $("#content_id_"+ID).find(".email-area").find(".to").val();
		var cc = $("#content_id_"+ID).find(".cc").parent(".email-area").find("div").length || $("#content_id_"+ID).find(".email-area").find(".cc").val();
		var cco = $("#content_id_"+ID).find(".cco").parent(".email-area").find("div").length || $("#content_id_"+ID).find(".email-area").find(".cco").val();
		var subject = $("#content_id_"+ID).find(".subject").val();
		var auxAttachments = false;
		var attachments = $("#content_id_"+ID).find("ul.attachments-list").find("li").each(function(){
			($(this).find("p").length > 0) ? auxAttachments = true : auxAttachments;
			if(auxAttachments)
				return false;
		});
		var textArea = RichTextEditor.getData("body_"+ID);
		
		if(!to && !cc && !cco && !auxAttachments && subject == "" && textArea == "")
			close_delete(ID, msg_sent);
	}
