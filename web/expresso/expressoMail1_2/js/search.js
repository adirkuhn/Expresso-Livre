	...
	{
		var treeFoldersSearch = new dFTree({name : 'treeFoldersSearch'});
		
		var root	= new dNode({id : 'root', caption: get_lang('My Folders'), onClick: "ttree.setFolder('root')"});
		
		treeFoldersSearch.add(root, 'root');
		...
		for(var i = 0; i < folders.length ; i++ )
		{
			...
			{
                var node_parent = treeFoldersSearch.getNodeById(folders[i].folder_parent);
                
                if( typeof node_parent != 'undefined')
                {
                	node_parent.plusSign = true;
                	treeFoldersSearch.alter(node_parent);
                }
            }
			
			treeFoldersSearch.add( nn, folders[i].folder_parent );
		}

		treeFoldersSearch.draw( $("#divFoldersSearch")[0] );
		treeFoldersSearch.getNodeById("root")._select();
		root.changeState();
	}
		...
		for(var j=0 ; j <= 11 ; j++)
		{
			...
			if (j == 1)
			{
				...
				if (aux.flag.match('T'))
				{
					...
					td1 = "<img src='templates/"+template+"/images/clip.gif'>";
				}
				else
					td1 = '';
			}
	...
	searchE.prototype.quickSearchMail = function(value, page, sort, border_id)
	{
		...
		var params		= {page: this.page, sort: this.sort, search: this.value};
		...
		for( var i = 0; i < nm_box.length; i++ )
		 ...
		 else
		 {
			    ...
			    var link = ' <a href="#" onclick="searchE.prototype.quickSearchAbort(); clean_msg(); return false;">cancelar<a/>';
			    ...
			    var handler = function( data )
			    {
					...
					if ( local_folders.length > 0 ){
						expresso_local_messages.setSortType('SORTDATE');
						var tmp = expresso_local_messages.search( local_folders, "##ALL <=>" + url_encode(value) + "##" ); 

						if( tmp.length > 0 )
							if( !allMsg['num_msgs'] ){
								clean_msg();
							    not_found_corrent_folder = false;
								if(tmp.length)
									allMsg['gears_num_msgs'] = tmp.length;
							}else
								allMsg['gears_num_msgs'] = tmp.length;
					...
					if( !allMsg['num_msgs'] && !url.length && not_found_corrent_folder){
						clean_msg();
						return EsearchE.viewLocalMessage();
					}
