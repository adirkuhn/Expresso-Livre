		...
		if(data_[index].type_contact == "G"){
			...
			if(ldap_id){
				draw_email_box(
					...
					, content.find(field_).filter("input")
				);
			}else{
				draw_email_box(final_contact, content.find(field_).filter("input"));
			}
		}else{
			draw_email_box((data_[index].id_contact ? data_[index].id_contact : data_[index].id), content.find(field_).filter("input"), (data_[index].id_contact ? true : "G"));
		}
		content.find(field_).filter("input").val("");
		button.onclick = function(){
