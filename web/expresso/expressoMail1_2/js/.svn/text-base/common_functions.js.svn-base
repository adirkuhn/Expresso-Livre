	...
	if (typeof(expresso_mail_sync) != "undefined" && expresso_mail_sync.working) {
		...
		return mess;
            ...
            for (var i = 0; i < BordersArray.length; i++) 
                    if(BordersArray[i].sequence > 0 && (!isNaN(BordersArray[i].sequence)&&parseInt(BordersArray[i].sequence) == BordersArray[i].sequence) )
...
function folderbox()
{
	...
	if( typeof(window.ttree) == 'undefined')
		setTimeout(function(){ folderbox(); }, 200 );
	else
		ttree.load();
}
...
function configureFollowupflag(){
	...
	for (var i=0; i<selectedMessageIds.length; i++) {
		...
		messages.push(onceOpenedHeadersMessages[folder_name][number]);
	}
...
function source_msg(id_msg,folder){
	...
	var handler_source = function(data){
		download_attachments(null, null, data, null,null,'fonte_da_mensagem.eml');
	}
...
function select_all_messages(select)
{
	...
	if (select)
	{
		for (i=0; i<len_main_list; i++)
		{
			...
			add_className(Element(main_list[i].id), 'selected_msg selected_shortcut_msg');
		}
	}
	else
	{
